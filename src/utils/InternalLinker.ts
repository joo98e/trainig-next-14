...
export default class InternalLinker {
  ...
  private static readonly MOVIE = new InternalLinker((movieId: string) => `/example/movies/${movieId}`)

  static getPathname<K extends InternalLinkType>(type: InternalLinkType, data: GetPathnameArguments[K]) {
    switch (type) {
      case 'home':
